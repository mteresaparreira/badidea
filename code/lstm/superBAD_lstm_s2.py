...
def executeModel_LSTM(df, results_directory, seed_value = 42, sequence_length = 1, units = 64, dropouts = 0.2, activations = 'softmax', losses = 'categorical_crossentropy', optimizers = 'adam', epochs = 100, batch_sizes = 32):
                        ...
                        for epoch in epochs:
                            for i, batch_size in enumerate(batch_sizes):
                                try:
                                    ...
                                    if search_count <= 2072:
                                        continue

                                    if activation == 'relu' and loss == 'categorical_crossentropy':
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write(f'-------------- SEARCH COUNT {search_count} --------------' + '\n')
                                            results_file.write(f'Model skipped for parameter combination : {sequence_length}_{unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}' + '\n')
                                        continue
                                    elif activation == 'relu' and loss == 'hinge' and optimizer == 'Adam':
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write(f'-------------- SEARCH COUNT {search_count} --------------' + '\n')
                                            results_file.write(f'Model skipped for parameter combination : {sequence_length}_{unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}' + '\n')
                                        continue
                                    elif activation == 'sigmoid' and loss == 'binary_crossentropy' and optimizer == 'Adam':
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write(f'-------------- SEARCH COUNT {search_count} --------------' + '\n')
                                            results_file.write(f'Model skipped for parameter combination : {sequence_length}_{unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}' + '\n')
                                        continue
                                    elif activation == 'sigmoid' and loss == 'hinge' and optimizer == 'Adam':
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write(f'-------------- SEARCH COUNT {search_count} --------------' + '\n')
                                            results_file.write(f'Model skipped for parameter combination : {sequence_length}_{unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}' + '\n')
                                        continue
                                    elif activation == 'softmax' and loss == 'binary_crossentropy' and optimizer == 'Adam':
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write(f'-------------- SEARCH COUNT {search_count} --------------' + '\n')
                                            results_file.write(f'Model skipped for parameter combination : {sequence_length}_{unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}' + '\n')
                                        continue
                                    elif activation == 'softmax' and loss == 'categorical_crossentropy' and optimizer == 'Adam':
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write(f'-------------- SEARCH COUNT {search_count} --------------' + '\n')
                                            results_file.write(f'Model skipped for parameter combination : {sequence_length}_{unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}' + '\n')
                                        continue
                                    elif activation == 'softmax' and loss == 'hinge' and optimizer == 'Adam':
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write(f'-------------- SEARCH COUNT {search_count} --------------' + '\n')
                                            results_file.write(f'Model skipped for parameter combination : {sequence_length}_{unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}' + '\n')
                                        continue
                                    elif activation == 'tanh' and loss == 'hinge' and optimizer == 'Adam':
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write(f'-------------- SEARCH COUNT {search_count} --------------' + '\n')
                                            results_file.write(f'Model skipped for parameter combination : {sequence_length}_{unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}_{batch_size}' + '\n')
                                        continue
                                    ...
                                    with tf.device('/GPU:0'):  # Use GPU 0
                                        ...
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write("\n")
                                            results_file.write(f"------------ BEGIN SEARCH : for Script 2 | LSTM Network ------------" + "\n")
                            ...
                            # do not create plots and write to results.txt log unless the following condition(s) is met
                            # This is done inorder to prevent redundancy in rewriting information (creating batch_size plots) that has already been logged during the previous execution of the script
                            if search_count <= 2072:
                                continue
                            if activation == 'relu' and loss == 'categorical_crossentropy':
                                continue
