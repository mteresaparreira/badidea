                        ...
                        for epoch in epochs:
                            for i, batch_size in enumerate(batch_sizes):
                                try:
                                    ...
                                    if search_count <= 1256:
                                        continue                                    

                                    # Based on manual observation of the performance of the model with the below parameters from the 'batch_size_results' graph plots
                                    # We do not train the model over those parameters
                                    if activation != 'softmax' or loss != 'categorical_crossentropy' or optimizer != 'Adam':
                                        print(f'IF CONDITION NOT MET {unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}')
                                        continue
                                    print(f'IF CONDITION MET {unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}')

                                    # if activation == 'softmax' and loss == 'categorical_crossentropy' and optimizer == 'Adam':
                                    #     print(f'IF CONDITION MET {unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}')
                                    #     continue
                                    # else:
                                    #     print(f'IF CONDITION NOT MET {unit}_{dropout}_{activation}_{loss}_{optimizer}_{epoch}')
                                    ...
                                    with tf.device('/GPU:0'):  # Use GPU 0
                                        ...
                                        with open(f'{results_directory}/results_LSTM.txt', 'a') as results_file:
                                            results_file.write("\n")
                                            results_file.write(f"------------ BEGIN SEARCH : for Script 2 ------------" + "\n")
